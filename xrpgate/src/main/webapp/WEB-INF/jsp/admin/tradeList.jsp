...
<!--타이틀시작-->
<div id="header3"><img src="/img/title0027.jpg" width="400" height="35" />
<!-- </div> -->
<!--타이틀끝-->
				                    ...
				                    <div class="bview_box">
										<ul>
											...
											<c:choose>
												...
												<c:otherwise>
												<div id ="statusNo">
													<li class="full"><strong><span>리플 처리 기준금액 (A)</span></strong><input type="number" id="resultUnitPrice" name="resultUnitPrice" value="0"></li>
													<li class="full"><strong><span>리플 처리 매매수량  (B)</span></strong><input type="number" id="resultQty" name="resultQty" value="0"></li>
													<li class="full"><strong><span>수수료 (C)</span></strong><input type="number" id="fee" name="fee" value="0"></li>
													<li class="full"><strong><span>총합계 (A*B+C)</span></strong><input type="number" id="resultTotMoney" name="resultTotMoney" readonly="readonly"><span id="balance"></span></li>
													<input type="button" id="popupBtn" class="btn-style"  value="처리 완료">
												</div>
												</c:otherwise>
											</c:choose>
				                        </ul>
...
<script type="text/javascript">
	function layer_open(el, seq){
		...
		if(status == "N"){
			//답변미등록
			$("#targetActionText").html("<br>본 리플매매 건을 완료 처리 하시겠습니까?");
			$("#statusYes").hide();              //조회 Area
			$("#statusNo").show();              //등록 Area
			$("#popupBtn").show();            //처리등록버튼
		}else{
			// 답변완료
			$("#statusYes").show();              //조회 Area
			$("#statusNo").hide();               //등록 Area
			$("#popupBtn").hide();             //처리등록버튼
		}
...
//////////////////////////////////////////////////////////////////////////////////////////////////
		...
		var doSubmit = function(){
			...
			var infoData = $("form[name=popupForm]").serialize();
	 		$.ajax({
			    url : "${contextPath}/admin/jsonInsertRippleTrade.do",
			    dataType : "json",
